...
CREATE FUNCTION Rooster_check() RETURNS trigger AS $Rooster_check$
	BEGIN
		...
		IF ((SELECT COUNT(Docent_Cursussen_cursuscode) FROM Rooster WHERE Docent_Cursussen_cursuscode = NEW.Docent_Cursussen_cursuscode AND datum = NEW.datum AND lestijd = NEW.lestijd) => 1) THEN
		RAISE EXCEPTION '% geeft al les op dat tijdstip', NEW.Docent_Cursussen_cursuscode;
		END IF;
